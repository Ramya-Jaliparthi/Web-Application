...
export class FilterComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  initConfig() {
    ...
    this.filterConfigClone = this.filterConfig;
    if (this._filterConfig.saveFilterState === true && this.globalService.filterState[this._filterConfig.filterStatePropName]) {
      ...
      if (this._filterConfig.closeAccordionOnBack) {
        this._filterConfig.items.forEach(item => {
          item.expanded = false;
        });
      }
  ...
  ngOnChanges(change) {
    setTimeout(() => {
      if (change.dispatchEvent) {
        if (change.dispatchEvent.currentValue === FilterComponentConstants.CLEAR || change.dispatchEvent.currentValue === 'UPDATE') {
          this.resetConfig();
        }
      }
    ...
    if (this._filterConfig.items.length > 0) {
      this._filterConfig.items.forEach((item, index) => {
        if (item.type === 'radio' || item.type === 'calendar') {
          if (!item.sortBy) {
            item.model = item.defaultModel ? item.defaultModel : '';
